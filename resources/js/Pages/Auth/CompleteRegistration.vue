...
<template>
    ...
    <form @submit.prevent="submit">
        ...
        <div class="mt-4">
            ...
            <ExclamationCircleIcon class="h-5 w-5 text-gray-500 inline mx-2"/>
            <breeze-input id="first_name" type="text" class="mt-1 block w-full" v-model="form.first_name" required
        ...
        <div class="mt-4">
            ...
            <ExclamationCircleIcon class="h-5 w-5 text-gray-500 inline mx-2"/>
            <breeze-input id="last_name" type="text" class="mt-1 block w-full" v-model="form.last_name" required
            ...
            <breeze-dropdown align="left" width="48">
                <template #trigger>
                    <span class="inline-flex rounded-md">
                        <button type="button"
                                class="inline-flex items-center mt-2 px-3 py-2 border text-sm leading-4 font-medium rounded-md text-gray-500 bg-white hover:text-gray-700 focus:outline-none transition ease-in-out duration-150">
                ...
                <template #content>
                    <DropdownItem v-for="locality in localities" :key="locality.id" href=""
                                  @click="selectLocality(locality)" as="button"> {{ locality.name }}
                    </DropdownItem>
                </template>
            </breeze-dropdown>
        ...
        <div class="flex items-center justify-between mt-4">
            <inertia-link :href="route('logout')" method="post"
                          class="underline text-sm text-gray-600 hover:text-gray-900">{{ lang.get('auth.logout') }}
            </inertia-link>
            <breeze-button class="ml-4" :class="{ 'opacity-25': form.processing }" :disabled="form.processing">
                {{ lang.get('auth.complete_registration') }}
            </breeze-button>
        </div>
    </form>
</template>
